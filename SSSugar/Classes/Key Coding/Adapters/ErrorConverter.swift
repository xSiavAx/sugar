...
///
class SSErrorConverter<ErrorT: Error> {
    ...
    static func stringToError(input: Any?, build: @escaping (_ string: String)->ErrorT?) -> ErrorT? {
        guard input != nil else { return nil }
        guard let error = input as? String else {
            customAssert("Unexpected dict error \(input!)")
    ...
    static func parser(build: @escaping (_ string: String)->ErrorT?) -> SSKeyField<ErrorT?>.Adapter {
...
extension SSErrorConverter where ErrorT: SSStringRepresentableError {
    ...
    static func parser() -> SSKeyField<ErrorT?>.Adapter {
